    ...
    with torch.no_grad():
        for subset in ('train', 'valid', 'test'):
            ...
            for batch_idx, batch in tqdm(enumerate(dataloader), total=num_batches, disable=(not verbose)):
                ...
                subset_results["mixture_dbfs"] += compute_dbfs_spec(x, SAMPLE_RATE, spec_params, device=device).squeeze().tolist()
                ...
                for label_idx, label in enumerate(train_dataset.labels):
                    ...
                    subset_results["isolated_" + label + "_dbfs"] += compute_dbfs_spec(source_maggram, SAMPLE_RATE, spec_params, device=device).squeeze().tolist()
                    subset_results["reconstructed_" + label + "_dbfs"] += compute_dbfs_spec(recon_source_maggram, SAMPLE_RATE, spec_params, device=device).squeeze().tolist()
